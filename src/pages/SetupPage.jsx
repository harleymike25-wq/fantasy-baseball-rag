  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto" }}>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        >
          Upload a PDF of your cocktail book (e.g. the Speakeater Codex). Claude will extract
          all recipes and store them in Supabase. This is a one-time operation.
        </p>
        <p
          style={{
            fontSize: "0.82rem",
        ...
        >
          For large PDFs (&gt;30 pages), use the CLI script instead to avoid timeouts:
          <br />
          <code
            style={{
              color: "var(--amber)",
        ...
        {loading ? (
          <div style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>
            ...
            Extracting recipes — this may take a minute…
          </div>
        ) : (
        ...
        {status && (
          <div
            style={{
              marginTop: 14,
              padding: "10px 14px",
              background: status.error ? "rgba(124,74,74,0.15)" : "var(--amber-dim)",
              borderRadius: "var(--radius)",
              fontSize: "0.88rem",
            }}
          >
            {status.error
              ? `Error: ${status.error}`
              : `✓ Ingested ${status.count} recipes`}
          </div>
        )}
      </div>

      {/* Step 3 – env vars reminder */}
      <div className="card">
