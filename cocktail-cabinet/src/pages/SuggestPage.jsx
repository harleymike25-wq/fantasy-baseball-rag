...
export default function SuggestPage({ cabinet }) {
  ...
  async function fetchSuggestions() {
    ...
    setResult(null);
    try {
      const data = await getSuggestions(cabinet, query.trim() || "What cocktails can I make?");
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: "1.2rem", fontWeight: 600, marginBottom: 4 }}>What can I make?</h1>
      ...
      {result && !hasSuggestions && (
        <div className="empty-state">
          ...
          <div className="empty-state__sub">Try ingesting a recipe book first (Setup tab)</div>
        </div>
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          {result.can_make?.length > 0 && (
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {result.can_make.map((s, i) => <SuggestionCard key={i} suggestion={s} type="can_make" />)}
              </div>
            </div>
          )}
          ...
          {result.almost_there?.length > 0 && (
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {result.almost_there.map((s, i) => <SuggestionCard key={i} suggestion={s} type="almost" />)}
              </div>
            </div>
          )}
          ...
          {result.bartender_note && (
            <div className="card" style={{ borderColor: "rgba(200,146,42,0.25)" }}>
              <div className="section-title" style={{ marginBottom: 10 }}>Bartender's Note</div>
              <div style={{ fontSize: "0.9rem", color: "var(--text-muted)", lineHeight: 1.7 }}>
                <ReactMarkdown remarkPlugins={[remarkGfm]}>{result.bartender_note}</ReactMarkdown>
              </div>
            </div>
          )}
        </div>
