...
import { identifyBottle } from "../lib/api";

const EMPTY_FORM = { brand: "", canonical_name: "", spirit_type: "", subcategory: "", origin: "" };
...
export default function CabinetPage({ cabinet }) {
  ...
  async function handlePhoto(base64, mimeType) {
    ...
    setError("");
    try {
      const data = await identifyBottle(base64, mimeType);
      if (data.error) {
        setError(data.error);
      } else {
        setIdentified(data);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const payload = { ...form, canonical_name: form.canonical_name || form.brand };
  ...
  if (loading) return <div className="empty-state"><div className="empty-state__icon">⌛</div></div>;
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        {adding
          ...
          : <button className="btn btn--primary" onClick={() => setAdding(true)}>+ Add Bottle</button>
        }
      </div>
        ...
        <div className="card" style={{ marginBottom: 24 }}>
          {!identified && (
            <div style={{ marginBottom: 20 }}>
              ...
              {identifying
                ? <div style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>Identifying bottle…</div>
                : <PhotoCapture onCapture={handlePhoto} />
              }
              <div style={{ color: "var(--text-muted)", fontSize: "0.78rem", marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--border)" }}>
          ...
          {identified && (
            <div style={{ marginBottom: 16, padding: "10px 14px", background: "var(--amber-dim)", borderRadius: "var(--radius)", fontSize: "0.85rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>
                Identified: <strong>{identified.canonical_name}</strong>
                {identified.confidence === "low" && <span style={{ color: "var(--amber)", marginLeft: 6 }}>(low confidence — verify below)</span>}
              </span>
              <button style={{ background: "none", border: "none", color: "var(--text-muted)", fontSize: "0.8rem", cursor: "pointer" }} onClick={() => setIdentified(null)}>Re-photo</button>
            </div>
          )}

          {error && <div style={{ color: "#c05050", fontSize: "0.85rem", marginBottom: 12 }}>{error}</div>}
          ...
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <button className="btn btn--primary" onClick={handleSave} disabled={!form.brand || !form.spirit_type || saving} style={{ marginTop: 4 }}>
        ...
        Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([type, items]) => (
          <div key={type} style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {items.map((b) => <BottleCard key={b.id} bottle={b} onRemove={() => removeBottle(b.id)} />)}
            </div>
          </div>
        ))
      )}
